...
public class GeminiAiService : IAiService
{
    ...
    private static string BuildGoalPlanPrompt(UserInterest goal, UserProfile? profile)
    {
        ...
        promptBuilder.AppendLine("4. Значения title и description — чистый текст без markdown-символов (**, `, #).");
    ...
    private async Task<string> BuildPromptAsync(int userId)
    {
        ...
        promptBuilder.AppendLine("5. Пиши ЧИСТЫМ ТЕКСТОМ без markdown-разметки: никаких **, `, #, __ и других служебных символов форматирования. Заголовки шагов — просто как обычный текст с двоеточием, без звездочек и решеток.");
        ...
        promptBuilder.AppendLine("СФОРМИРУЙ ОТВЕТ СТРОГО ПО СЛЕДУЮЩЕЙ СТРУКТУРЕ (без markdown, просто обычным текстом):");
