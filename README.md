...
**База данных**
- PostgreSQL. Миграций в репозитории нет — схема управляется вручную (см. раздел "База данных" ниже).
...
Создайте базу данных и накатите схему (миграций в проекте нет, актуальная схема описывается вручную по мере развития — см. `Infrastructure/AppDbContext.cs` за списком таблиц и колонок).
...
Схема управляется вручную: в репозитории нет каталога миграций EF Core, а `AppDbContext` описывает маппинг на уже существующие таблицы Postgres (snake_case-колонки через Fluent API). При изменении домена схему нужно обновлять в базе отдельно и синхронизировать маппинг в `AppDbContext.cs`.
